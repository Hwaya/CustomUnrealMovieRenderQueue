...
namespace
{
	...
	static void RegisterTabImpl()
	{
		...
		MRPQueueTabSpawner
			.SetGroup(WorkspaceMenu::GetMenuStructure().GetLevelEditorCinematicsCategory())
			.SetDisplayName(/*IMovieRenderPipelineEditorModule::MoviePipelineQueueTabLabel*/FText::FromString(TEXT("Custom Movie Render Queue")))
			.SetTooltipText(LOCTEXT("CustomRenderTab_Tooltip", "Open the Movie Render Queue to render Sequences to disk at a higher quality than realtime allows."))
