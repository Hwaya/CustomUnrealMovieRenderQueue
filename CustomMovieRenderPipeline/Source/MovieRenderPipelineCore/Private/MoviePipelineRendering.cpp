...
void UMoviePipeline::RenderFrame()
{
	...
	FVector tempPrevViewLocation = FrameInfo.CurrViewLocation;
	FRotator tempPrevViewRotation = FrameInfo.CurrViewRotation;
	...
	if (CachedOutputState.IsFirstTemporalSample())
	{
		...
		{
			FMoviePipelineFrameOutputState TempOutputState = CachedOutputState;
			TempOutputState.StereoState = EStereoState::Left;
			FMoviePipelineMergerOutputFrame& LeftOutputFrame = OutputBuilder->QueueOutputFrame_GameThread(TempOutputState);
			TempOutputState.StereoState = EStereoState::Right;
			FMoviePipelineMergerOutputFrame& RightOutputFrame = OutputBuilder->QueueOutputFrame_GameThread(TempOutputState);

			for(UMoviePipelineRenderPass* RenderPass : InputBuffers)
			{
				RenderPass->GatherOutputPasses(LeftOutputFrame.ExpectedRenderPasses);
				RenderPass->GatherOutputPasses(RightOutputFrame.ExpectedRenderPasses);
			}
		}
		else
		{
	...
	for (int32 TileY = 0; TileY < TileCount.Y; TileY++)
	{
		for (int32 TileX = 0; TileX < TileCount.X; TileX++)
		{
			...
			for (int32 RenderSampleIndex = 0; RenderSampleIndex < NumSamplesToRender; RenderSampleIndex++)
			{
				...
				if (OutputSettings->bUseStereoMode)
				{
					float TempDistanceOfEyes = 0.f;
					float TempDistanceOfFocus = 0.f;
					bool TempbUseFocus = false;
					...
					for (auto BindObject : BoundObjects)
					{
						if (BindObject.GetName().Contains(TEXT("StereoCamera")))
						{
							...
							{
								FString TrackDisplayName = Track->GetDisplayName().ToString();
								UE_LOG(LogMovieRenderPipeline, Log, TEXT("%s"), *TrackDisplayName);

								FFrameTime TempTime = FFrameTime(FFrameRate::TransformTime(FFrameTime(FFrameNumber(FrameIndex)), TargetSequence->GetMovieScene()->GetDisplayRate(), TargetSequence->GetMovieScene()->GetTickResolution()).FloorToFrame());

								// Try cast to Float Track
								UMovieSceneFloatTrack* FloatTrack = Cast<UMovieSceneFloatTrack>(Track);
								if (FloatTrack)
								{
									UE_LOG(LogMovieRenderPipeline, Log, TEXT("FloatCast Success"));
									TArray<UMovieSceneSection*> MovieSceneSections = FloatTrack->GetAllSections();
									if (MovieSceneSections.Num())
									{
										...
										if (FloatSection)
										{
											...
											float Value;
											Channels[0]->Evaluate(TempTime, Value);
											if (TrackDisplayName.Contains(TEXT("Eye")))
											{
												TempDistanceOfEyes = Value;
												UE_LOG(LogMovieRenderPipeline, Log, TEXT("%s : %f"), *Track->GetDisplayName().ToString(), Value);
											}
											else if (TrackDisplayName.Contains(TEXT("Focus")))
											{
												TempDistanceOfFocus = Value;
												UE_LOG(LogMovieRenderPipeline, Log, TEXT("%s : %f"), *Track->GetDisplayName().ToString(), Value);
											}
										}
									}
								...
								else
								{
									...
									if (BoolTrack)
									{
										UE_LOG(LogMovieRenderPipeline, Log, TEXT("BoolCast Success"));
										TArray<UMovieSceneSection*> MovieSceneSections = BoolTrack->GetAllSections();

										if (MovieSceneSections.Num())
										{
											UMovieSceneBoolSection* BoolSection = nullptr;
											BoolSection = CastChecked<UMovieSceneBoolSection>(MovieSceneSections[0]);
											if (BoolSection)
											{
												...
												if (TrackDisplayName.Contains(TEXT("Parallel")))
												{
													TempbUseFocus = Value;
													UE_LOG(LogMovieRenderPipeline, Log, TEXT("%s : %s"), *Track->GetDisplayName().ToString(), Value ? TEXT("True") : TEXT("False"));
												}
											}
										}
									}
					...
					SampleState.DistanceOfFocus = TempDistanceOfFocus;


					float CorrectAngle = 0.f;
					CorrectAngle = atan2(TempDistanceOfEyes / 2.f, TempDistanceOfFocus) * (180.f / PI);

					FMoviePipelineRenderPassMetrics LeftState = SampleState;
					...
					{
						LeftState.FrameInfo.PrevViewLocation = LeftState.FrameInfo.CurrViewLocation;
						LeftState.FrameInfo.PrevViewRotation = LeftState.FrameInfo.CurrViewRotation;
						RightState.FrameInfo.PrevViewLocation = RightState.FrameInfo.CurrViewLocation;
						RightState.FrameInfo.PrevViewRotation = RightState.FrameInfo.CurrViewRotation;
					}
					else // ¾Èµ¤¾î¾º¿ì´Â°Å
					{
				...
				{
					SampleState.StereoState = EStereoState::None;
					// Now we can request that all of the engine passes render. The individual render passes should have already registered delegates
					// to receive data when the engine render pass is run, so no need to run them.
					for (TSharedPtr<MoviePipeline::FMoviePipelineEnginePass> EnginePass : ActiveRenderPasses)
					{
